...
//Basic checks
if (!$thisclient || !$_GET['id'] || !$_GET['h'] || !($attachment = Attachment::lookup($_GET['id'])) || !($file = $attachment->getFile())) {
...
if (strcasecmp(trim($_GET['h']), $vhash) || !($ticket = $attachment->getTicket()) || !$ticket->checkUserAccess($thisclient)) {
    Http::response(404, __('Unknown or invalid file'));
}
//Download the file..
$file->download();
