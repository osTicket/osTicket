...
?>
<hr/>
<?php if (($users = $ticket->getCollaborators())) : ?>
    <div id="manage_collaborators">
        <form method="post" class="collaborators" action="#tickets/<?= $ticket->getId(); ?>/collaborators">
            <table border="0" cellspacing="1" cellpadding="1" width="100%">
                ...
                foreach ($users as $user) {
                    ...
                    echo sprintf('<tr>
                        ...
                        <td width="30">&nbsp;</td>
                    </tr>', $user->getId(), $user->getId(), $checked, $user->getId(), Format::htmlchars($user->getName()), $user->getEmail(), $user->getId(), $user->getId());
                }
                ?>
            </table>
            <hr style="margin-top:1em"/>
            <div><a class="collaborator"
                    href="#tickets/<?= $ticket->getId(); ?>/add-collaborator"
                    ><i class="icon-plus-sign"></i> <?= __('Add New Collaborator'); ?></a></div>
            <div id="savewarning" style="display:none; padding-top:2px;"><p
                    id="msg_warning"><?= __('You have made changes that you need to save.'); ?></p></div>
            <p class="full-width">
...
if ($_POST && $ticket && $ticket->getNumCollaborators()) :
    $recipients = sprintf(__('Recipients (%d of %d)'), $ticket->getNumActiveCollaborators(), $ticket->getNumCollaborators());
    ?>
