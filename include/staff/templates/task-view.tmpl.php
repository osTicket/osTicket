...
<div class="clear tixTitle">
	<div class="pull-right flush-right">
		...
		<?php
			...
			<a href="tasks.php?id=<?php echo $task->getId(); ?>"
	   ...
	   <?php
		...
		if ($ticket) { ?>
			...
			<a id="reload-task" class="preview"
				...
				echo sprintf('href="tasks.php?id=%d" ', $task->getId());
...
<div class="flex" id="task_view_flex">
	<div class="clear"></div>
	<div id="task_thread_flex_container">
	...
	<?php
	if (!$ticket) { ?>
		<div id="task_info_flex_container" class="sticky tile">
			<div class="ticket_number">
				<h5>
					<?php echo sprintf(__('Task #%s'), $task->getNumber()); ?>
				</h5>
			</div>
			...
			foreach (DynamicFormEntry::forObject($task->getId(),
				...
				?>
				<br>
				<table class="ticket_info ticket_content custom-data" cellspacing="0" cellpadding="0" width="940" border="0">
				<thead>
...
<?php }elseif($warn) { ?>
	<div id="ticket_info_msg_warning"><?php echo $warn; ?></div>
<?php }
...
if (!$ticket) { ?>
	<div id="task_response_options" class="<?php echo $ticket ? 'ticket_task_actions' : ''; ?> sticky bar stop actions">
		...
		<?php
		if ($role->hasPerm(TaskModel::PERM_REPLY)) { ?>
		<form id="task_reply" class="ticket_content tab_content spellcheck save"
		...
		<form id="task_note"
			action="<?php echo $action; ?>"
			class="ticket_content tab_content spellcheck save <?php 
	...
	<?php echo $reply_attachments_form->getMedia(); ?>
<?php 
} ?>
