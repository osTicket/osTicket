...
<div class="pull-left" style="width:700px;">
    <form action="users.php" method="get">
        ...
        <table>
            <tr>
                <td><input type="text" id="basic-user-search" name="query" size=30 value="<?= Format::htmlchars($_REQUEST['query']); ?>"
                autocomplete="off" autocorrect="off" autocapitalize="off"></td>
                <td><input type="submit" name="basic_search" class="button" value="<?= __('Search'); ?>"></td>
 ...
 <div class="pull-right flush-right" style="padding-right:5px;">
    <b><a href="#users/add" class="Icon newstaff popup-dialog"><?= __('Add User'); ?></a></b>
    |
...
<form action="users.php" method="POST" name="staff" >
 ...
 <table class="list" border="0" cellspacing="1" cellpadding="0" width="940">
    ...
    <thead>
        <tr>
            <th width="350"><a <?= $name_sort; ?> href="users.php?<?= $qstr; ?>&sort=name"><?= __('Name'); ?></a></th>
            <th width="250"><a  <?= $status_sort; ?> href="users.php?<?= $qstr; ?>&sort=status"><?= __('Status'); ?></a></th>
            <th width="100"><a <?= $create_sort; ?> href="users.php?<?= $qstr; ?>&sort=create"><?= __('Created'); ?></a></th>
            <th width="145"><a <?= $update_sort; ?> href="users.php?<?= $qstr; ?>&sort=update"><?= __('Updated'); ?></a></th>
        </tr>
    </thead>
    <tbody>
    <?php
...
?>
               <tr id="<?= $row['id']; ?>">
                <td>&nbsp;
                    <a class="userPreview" href="users.php?id=<?= $row['id']; ?>"><?php
                        echo Format::htmlchars($name); ?></a>
                    &nbsp;
                    <?php
        if ($row['tickets']) {
            echo sprintf('<i class="icon-fixed-width icon-file-text-alt"></i><small>(%d)</small>', $row['tickets']);
        }
        ?>
                </td>
                <td><?= $status; ?></td>
