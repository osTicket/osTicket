    ...
    <form action="directory.php" method="GET" name="filter">
       <input type="text" name="q" value="<?php echo Format::htmlchars($_REQUEST['q']); ?>" >
        <select name="did" id="did">
             ...
             <?php
             $sql='SELECT dept.id, dept.name as dept,count(staff.staff_id) as users  '.
                  'FROM '.DEPT_TABLE.' dept '.
                  'INNER JOIN '.STAFF_TABLE.' staff ON(staff.dept_id=dept.id AND staff.isvisible=1) '.
                  'GROUP By dept.id HAVING users>0 ORDER BY dept.name';
