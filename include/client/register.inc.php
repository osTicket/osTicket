...
if (isset($user) && $user instanceof ClientCreateRequest) {
    $bk = $user->getBackend();
    $info = array_merge($info, array('backend' => $bk::$id, 'username' => $user->getUsername(),));
}
$info = Format::htmlchars(($errors && $_POST) ? $_POST : $info);
?>
<h1><?= __('Account Registration'); ?></h1>
<p><?= __('Use the forms below to create or update the information we have on file for your account'); ?>
</p>
<form action="account.php" method="post">
    <?php csrf_token(); ?>
    <input type="hidden" name="do" value="<?= Format::htmlchars($_REQUEST['do'] ? : ($info['backend'] ? 'import' : 'create')); ?>" />
    <table width="800" class="padded">
        ...
        <tr><td colspan=2"><div><hr><h3><?= __('Access Credentials'); ?></h3></div></td></tr>
        <?php if ($info['backend']) : ?>
            <tr>
                <td width="180"><?= __('Login With'); ?>:</td>
                <td>
        ...
        <?php else: ?>
            <tr>
                <td width="180"><?= __('Create a Password'); ?>:</td>
                <td>
    ...
    <p style="text-align: center;">
        ...
        <input type="button" value="Cancel" onclick="javascript: indow.location.href = 'index.php';"/>
    </p>
</form>
