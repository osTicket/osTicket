...
<table class="table">
    <tbody>
      <?php
      foreach (User2FABackend::allRegistered() ?: array() as $bk) {
        if ($user instanceof User) {
            $user = $user->getAccount();
        }
          $configuration = $user->get2FAConfig($bk->getId());
      ...
      <tr id="<?php echo $bk->getId(); ?>" class="2fa-type <?php echo $vclass; ?>">
        ...
        <td width="300px">
            <a class="config2fa"
                href="<?php echo sprintf('#users/%d/2fa/configure/%s',
                $user->getId(), urlencode($bk->getId())); ?>"> <?php echo $bk->getName(); ?>
              </a>
...
if ($auth && $form) {
 ...
 <br>
<form class="bk" method="post" action="<?php echo sprintf('#users/%d/2fa/configure/%s',
