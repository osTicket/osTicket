...
?>
<h1><strong><?= $category->getName() ?></strong></h1>
<p><?= Format::safe_html($category->getDescription()); ?></p>
<hr>
<?php
...
if (($res = db_query($sql)) && db_num_rows($res)) {
    ...
    while ($row = db_fetch_array($res)) {
        ...
        echo sprintf('
            <li><a href="faq.php?id=%d" >%s &nbsp;%s</a></li>', $row['faq_id'], Format::htmlchars($row['question']), $attachments);
    }
    echo '  </ol>
         </div>
