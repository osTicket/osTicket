...
<form action="index.php" method="get" id="kb-search">
    ...
    <div>
        <input id="query" type="text" size="20" name="q" value="<?= Format::htmlchars($_REQUEST['q']); ?>">
        <select name="cid" id="cid">
            <option value="">&mdash; <?= __('All Categories'); ?> &mdash;</option>
            <?php
            ...
            if (($res = db_query($sql)) && db_num_rows($res)) {
                while ($row = db_fetch_array($res)) {
                    echo sprintf('<option value="%d" %s>%s (%d)</option>', $row['category_id'], ($_REQUEST['cid'] && $row['category_id'] == $_REQUEST['cid'] ? 'selected="selected"' : ''), $row['name'], $row['faqs']);
                }
            }
            ?>
        </select>
        <input id="searchSubmit" type="submit" value="<?= __('Search'); ?>">
    </div>
    <div>
        <select name="topicId" id="topic-id">
            ...
            if (($res = db_query($sql)) && db_num_rows($res)) {
                while ($row = db_fetch_array($res)) {
                    echo sprintf('<option value="%d" %s>%s (%d)</option>', $row['topic_id'], ($_REQUEST['topicId'] && $row['topic_id'] == $_REQUEST['topicId'] ? 'selected="selected"' : ''), $row['helptopic'], $row['faqs']);
                }
            }
            ?>
        </select>
...
<div>
    ...
    if ($_REQUEST['q'] || $_REQUEST['cid'] || $_REQUEST['topicId']) { //Search.
        ...
        if (($res = db_query($sql)) && ($num = db_num_rows($res))) {
            ...
            while ($row = db_fetch_array($res)) {
                echo sprintf('
                <li><a href="faq.php?id=%d" class="previewfaq">%s</a></li>', $row['faq_id'], $row['question'], $row['ispublished'] ? __('Published') : __('Internal'));
            }
            echo '  </ol>
             </div>';
        } else {
