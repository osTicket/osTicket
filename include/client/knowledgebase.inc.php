...
<form action="index.php" method="get" id="kb-search">
    ...
    <div>
        <select name="topicId" id="topic-id">
            <option value="">&mdash; Tous les articles d'aide &mdash;</option>
            <?php
...
if($_REQUEST['q'] || $_REQUEST['cid'] || $_REQUEST['topicId']) { //Search.
    ...
    echo "<div><strong>Résultats de recherche</strong></div><div class='clear'></div>";
    if(($res=db_query($sql)) && ($num=db_num_rows($res))) {
        echo '<div id="faq">'.$num.' FAQs qui correspondent à votre recherche.
                <ol>';
        while($row=db_fetch_array($res)) {
            echo sprintf('
        ...
        echo '<strong class="faded">La recherche n\'a pas trouvé de FAQ qui corresponde à votre recherche.</strong>';
    }
    ...
    $sql='SELECT cat.category_id, cat.name, cat.description, cat.ispublic, count(faq.faq_id) as faqs '
        ...
        .' ORDER BY cat.name';
    if(($res=db_query($sql)) && db_num_rows($res)) {
        echo '<div>Cliquez sur la catégorie pour parcourir la FAQ.</div>
                <ul id="kb">';
        while($row=db_fetch_array($res)) {
